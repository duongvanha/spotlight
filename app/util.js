...
utils.highlightTopSuggestion = () => {
  if (window.searchResultsList.children[0]) {
...
utils.SetLoading = (isLoading) => {
  if (window.searchResultsList.children[0]) {
    if (isLoading) {
      window.searchResultsList.children[0].classList.add("loading")
    } else {
      window.searchResultsList.children[0].classList.remove("loading")
    }
  }
}
...
utils.createSuggestionElement = function (suggestion) {
  const isObject = Object.prototype.toString.call(suggestion) === '[object Object]'
  if (isObject) {
    const element = (
      <li className="cLauncher__suggestion" onClick={() => {
        utils.SetLoading(true)
        Promise.resolve(suggestion.action()).finally(() => {
...
utils.handleThemeInputValueChanges = async (event) => {
  const SpotlightCommandName = window.selectedElement.querySelector('.cLauncher__suggestion-title').innerText; //This is not the value inside of the search input box.
